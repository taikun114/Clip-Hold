...
> [!NOTE]
> このバージョンでは、Spotlight検索とコード検出に対応するため、アップデート後に起動するとバックグラウンドでインデックスの作成が行われます。履歴や定型文の数によってはインデックスの作成に長い時間がかかることがあります。\
> インデックスの作成が終わるまでは高い負荷がかかることがありますが、正常な動作ですのでご安心ください。インデックスの作成中でもClip Holdの機能は通常通りご利用いただけます。

### 新機能
#### 全般
- **コード検出に対応**
  - 項目がコードであるかどうかを検出できるようになりました。判別精度は100%ではありませんが、高精度で判別されるように設計されています。もし誤判定に気がついた場合はフィードバックをお送りください。
  - 判別できるコードは次の通りです: `Swift`、`JavaScript`、`Python`、`HTML / XML`、`CSS`、`JSON`、`YAML`、`TOML`、`Markdown`、`GraphQL`、`環境変数 (.env)`、`Rust`、`Go`、`C / C++`、`Java / Kotlin`、`SQL`、`シェルスクリプト`
- **Spotlight検索とショートカットアクションに対応**
  - 履歴と定型文をSpotlightから検索・コピーしたり、ショートカットアクションを使って特定の履歴や定型文をコピーしたりすることができます。
- **履歴でファイルを含むインポートとエクスポートに対応**
  - 保存フォルダに保存されているファイルやフォルダを含むすべての履歴データを書き出すことができるようになりました。引き続き以前のバージョンでエクスポートされた履歴ファイルをインポートすることも可能です。
- **「クイックオーバーレイ」機能を追加**
  - ショートカットキーを押している間、またはマウスカーソルで画面の端に触れている間に表示され、簡単操作で履歴や定型文をコピーできるクイックオーバーレイ機能が追加されました。ショートカットキーを押しながら表示する際は、マウスカーソルを動かしてショートカットキーを離すだけでアクションが実行されるため、わざわざクリックしなくても簡単に操作できるように設計されています。
  - クイックオーバーレイは一般設定の「クイックオーバーレイ」セクションからオンにすることができます。詳しい使い方は、一般設定の「クイックオーバーレイ」セクションにある「クイックオーバーレイの使い方...」ボタンからご覧いただけます。
- **「項目を変更してコピー」にテキスト置換機能を追加**
  - 「項目を変更してコピー」の入力欄の下にある「テキスト置換」ボタンをクリックするか`⌘ (Command)` + `F`キーを押すことでテキスト置換画面が開き、繰り返し登場するテキストを検索して置き換えたり、正規表現を使ってパターンを置き換えたりすることができます。
- **一般設定に「直前のテキストフィールドにクイックペースト」オプションを追加**
  - この設定を使用すると、履歴や定型文ウィンドウからコピー・標準テキストとしてコピー・変更してコピーしたときに、直前までフォーカスされていたアプリにフォーカスを戻してからクイックペーストを実行することができます。テキストフィールドにフォーカスした状態であれば、直前のテキストフィールドに簡単にコピーしてペーストすることができます。
  - `⌥ (Option)`キーを押している間は一時的にクイックペーストを無効化することができます。
- **開発者向け機能設定に「空白・改行記号を表示」設定を追加**
  - この設定をオンにすると、履歴と定型文に含まれる半角スペース、全角スペース、改行、タブが記号で薄く表示されるようになります。

#### コピー履歴
- **新規コピー機能を追加**
  - デフォルトでは`⌥ (Option)` + `⌘ (Command)` + `A`キーを押して、お好みのテキストを入力して新たなコピーを作成することができます。特定の文字列をコピーしたいときにメモなどに入力する必要がなくなりました。
- **コピー履歴のピン留め機能を追加**
  - ピン留めは同時に1つまで可能です。ピン留めされた履歴は、デフォルトでは`⌥ (Option)` + `⌘ (Command)` + `P`キーを押してコピー・クイックペーストできます。
- **コピー履歴設定に保存フォルダの総容量を再計算する機能を追加**
  - 保存フォルダに含まれるすべてのファイルとフォルダの容量を再計算して、正しい容量が表示できるようになりました。

#### 定型文
- **一般設定の「メニュー」セクションに「現在のプリセットアイコンを表示する」オプションを追加**
  - オンにすると、現在選択されているプリセットアイコンがメニューバーに表示されるようになります。

#### 履歴と定型文ウィンドウ
- **履歴ウィンドウに「このアプリからの他の履歴を表示」機能を追加**
  - 各履歴項目のコンテキストメニューから、簡単にアプリでフィルタリングできるようになりました。
- **履歴ウィンドウのフィルターメニューに「すべてのフィルターを解除」ボタンを追加**
  - 何らかのフィルターが適用されているときに表示され、クリックするとすべてのフィルターを解除することができます。

### バグ修正と改善
#### 全般
- **クイックペースト時にクラッシュする可能性があった問題を修正**
- **macOS Golden Gateで一部のシートが正しく表示されなかった問題を修正**
- **履歴や定型文の一部設定がアプリを再起動するまで反映されなかった問題を修正**
- **古いmacOSでメニューバーアイコンが透明になってしまうことがあった問題を修正**
- **改行のみのテキストをコピーしたときの履歴項目の高さが大きくなってしまう問題を修正**
- **設定項目を改善**
  - 一般設定にあった「定型文ウィンドウ」セクションと「履歴ウィンドウ」セクションはそれぞれ定型文設定とコピー履歴設定に移動しました。
- **macOS Golden Gateでのアイコン表示を改善**
- **除外するアプリの追加画面を改善**
- **macOS Tahoe以降でのプリセットピッカー表示を改善**
- **現在設定されているショートカットキーがメニューに表示されるように改善**

#### コピー履歴
- **クリップボードの変更検知時にクラッシュする可能性があった問題を修正**
- **大容量ファイルコピー時のアラート表示中に処理が止まる問題を修正**
  - ネイティブのアラートではOS側の制約により、アラートが表示されているときにすべての処理が止まっていましたが、これを修正するためにカスタムアラートを実装しました。これにより、アラートが表示されていても処理が止まらなくなり、アラートは常に最前面に表示されるようになりました。
- **ブラウザなどから画像をコピーしたときに正しいサムネイル画像が表示されない問題を修正**
- **ファイルコピー時の履歴追加パフォーマンスを改善**
- **ファイルコピーの動作を改善**
  - 大きなファイルをコピーしたときに進捗を表すプログレスバーが表示されるようになり、重複検知システムも強化されました。大きなファイルのコピー中、コピーを途中でキャンセルすることもできるようになりました。
- **フォルダコピー時の動作を改善**
  - フォルダをコピーしたときにアラートに表示される容量が正しく計算されるようになり、追加された容量計算のタイムアウト設定時間を超えたときにアラートが表示されるようになりました。さらに、履歴リストでフォルダコピーの進捗が表示されるようになりました。
- **より正確なコピー元アプリが検出されるように改善**
  - 別のアプリがフォーカスされているとき、背後のウィンドウから右クリックしてコピーした時などに、より正確なアプリが検出されるようになりました。ただし、システムの制約により、Clip Holdウィンドウ上で（アラートやシートなどの）コンテキストメニューからテキストをコピーした場合は別の最前面のアプリが検出されるためご注意ください（コピー履歴や定型文のコピーは正しく認識されますのでご安心ください）。
- **メニューからクリップボード監視を切り替えられるように改善**
- **既に保存されている履歴よりも少ない保存数に変更しようとしたときにアラートが表示されるように改善**
- **「コピーアラートを表示する容量」のデフォルトを100 MBに変更**
  - 気づかないうちに保存フォルダの容量が肥大化することを防止するため、アラート表示までの容量を減らしました。

#### 定型文
- **インポートの競合解決完了時にアプリがクラッシュすることがあった問題を修正**
- **プリセットアイコンピッカーのパッケージを変更**
  - 新たにアイコン名が表示されるようになり、カテゴリピッカーから特定のカテゴリに絞り込むことができるようになりました。

#### 履歴と定型文ウィンドウ
- **履歴ウィンドウから非常に長いテキストの履歴をコピーしたときにフリーズ・メモリ使用率増加が発生する可能性があった問題を修正**
- **定型文ウィンドウのメニューからコピーしたときにコピー通知が消えなくなる問題を修正**
- **macOS Sonomaの履歴ウィンドウでフィルターメニューにアプリアイコンが正しく表示されないことがあった問題を修正**
- **履歴ウィンドウのパフォーマンスを改善**
  - 初回表示時には100件までに制限され、スクロールすることで追加の履歴が読み込まれるページネーションに対応しました。
- **履歴ウィンドウで特定のファイル履歴のみ削除できるように改善**
  - 履歴項目がファイルの場合、`⌥ (Option)`キーを押しながらコンテキストメニューを開いたときに表示される「この項目のみ削除...」をクリックすることで、関連する他の履歴に影響を与えずにリストから削除できるようになりました。
- **macOS Golden Gateでの履歴と定型文ウィンドウの表示を改善**
- **履歴と定型文ウィンドウの操作性を改善**
  - 履歴と定型文ウィンドウを開いた時、最初の項目にフォーカスが当たるようになったため、キーボードだけで簡単に操作できるようになりました。そのまま文字を入力して検索したりリストにフォーカスを戻したりすることもできます。
- **履歴と定型文ウィンドウの検索欄の動作を改善**
  - 以前はエスケープキーを押すとウィンドウが閉じていましたが、検索欄に何らかの文字が入っている場合、エスケープキーを1回押すと内容がクリアされ、もう一度押すとウィンドウが閉じるようになりました。
- **「アプリアイコンを表示」設定を削除**
  - 履歴ウィンドウのアイコンにアプリアイコンを表示するかどうかを切り替えるものでしたが、表示されるアプリアイコンは非常に小さく、設定項目が煩雑になっていたため削除しました（アプリアイコンは常に表示されるようになります）。

## 1.6.3
### 新機能
- **生成AIによるフランス語（`fr`）、ドイツ語（`de`）、スペイン語（`es`）のローカライズを追加**
  - 私は日本語と英語以外の知識が全くないため、おかしな翻訳が含まれている可能性があります。修正すべき翻訳に気がついた方は、どこをどのように変更すべきかフィードバックをお送りいただけると助かります！
- **一般設定の「メニュー」セクションと「履歴ウィンドウ」セクションに「日付と時刻の表示方法」オプションを追加**
  - コピーされた日付の表示方法を変更します。次のオプションから選ぶことができます。
    - **絶対的**（デフォルト）\
      例: 2025/11/30 12:34
    - **相対的**\
      例: 5分前
    - **両方: 絶対的 (相対的)**\
      例: 2025/11/30 12:34 (5分前)
    - **両方: 絶対的 - 相対的**\
      例: 2025/11/30 12:34 - 5分前
    - **両方: 相対的 (絶対的)**\
      例: 5分前 (2025/11/30 12:34)
    - **両方: 相対的 - 絶対的**\
      例: 5分前 - 2025/11/30 12:34

### バグ修正と改善
- **クリップボード監視一時停止中にアプリを起動したときに送信される通知パネルに「再開」ボタンが表示されない問題を修正**
- **デフォルトプリセット編集画面のプリセット名がローカライズされていない問題を修正**
- **「履歴を変更してコピー」を「項目を変更してコピー」に修正**
  - 定型文の変更してコピー画面でも「履歴を変更してコピー」と表示されていた問題が修正されました。
- **履歴と定型文ウィンドウがエスケープキーで閉じられないことがあった問題を修正**
- **デフォルトプリセットのアイコンが生成されなかった問題を修正**
- **アプリのパフォーマンスを改善**
  - キャッシュの適切な利用と不要になったデータの自動クリーンアップにより、長時間Clip Holdを起動し続けた状態でもパフォーマンスが低下しにくくなりました。
- **履歴を変更してコピー画面のショートカットキーを改善**
  - 履歴を変更してコピー画面を開いているときに`⌘ (Command)` + `S`キーを押すとコピーアクションが実行されるようになりました。
- **システム設定で「コントラストを上げる」がオンの時に履歴と定型文の変更してコピー画面、定型文の追加編集画面の入力欄の枠を強調するように改善**
- **設定されていない定型文のショートカットキーが押された場合に通知を送信するように改善**
- **システム設定で「カラー以外で区別」がオンの時のプライバシー設定にあるクリップボード監視インジケーターを改善**
- **コピー履歴設定の「保存フォルダの総容量:」に、サイズ計算に関する説明を追加**
- **履歴と定型文の追加編集画面にある入力欄の高さ制限を削除**
  - これらの画面をウィンドウとして表示している時、ウィンドウの高さを大きくしても、入力欄をそれに合わせて大きくなるように修正されました。

## 1.6.2
### バグ修正と改善
- **正しいフォルダがコピーできないことがあった問題を修正**
  - この問題は重複検知機能の誤作動により発生していましたが、フォルダの場合は重複検知をスキップすることでこの問題を修正しました。
  - フォルダの重複検知が行われなくなったことにより、別のアプリから同じフォルダを何度もコピーすると重複して保存されてしまうためご注意ください。
- **プリセット名が空の状態で保存できてしまう問題を修正**
- **プリセットが切り替わったときに定型文ウィンドウが最も上にスクロールするように改善**
  - 項目が長いプリセットから切り替えた際にリストが切れてしまうことがあった問題が修正されました。
- **新しいプリセットが追加されたときに、追加されたプリセットに切り替わるように改善**
- **定型文追加画面のフォーカス管理を改善**
  - カスタムタイトル入力欄でReturnキーを押すと、本文の入力欄にフォーカスが移動するようになりました。
- **定型文追加画面のショートカットキーを改善**
  - 定型文追加画面を開いているときに`⌘ (Command)` + `S`キーを押すと保存アクションが実行されるようになりました。
...
- **定型文ウィンドウのコンテキストメニューとアクションメニューに「変更してコピー...」機能を追加**
  - 履歴ウィンドウの「変更してコピー...」（旧「編集してコピー...」）と同様に、一部分を編集して新たにコピーすることができる機能が定型文ウィンドウにも追加されました。
...
- **履歴ウィンドウの「編集してコピー...」を「変更してコピー...」に変更**
  - 機能の名前が変更され、よりわかりやすくなりました。機能に変更はありません。
- **「変更してコピー」画面のテキスト入力欄を等幅フォントに変更**
...
- **「開発者向け機能」タブの追加**
  ...
  - **「カラーコードでフィルタリングできるようにする」オプションを追加**
    - 履歴をウィンドウのフィルタリングオプションに「カラーコードのみ」を追加します。
